...
1. [싱글톤 패턴](#싱글톤-패턴singleton) 클래스가 하나의 인스턴스만 갖도록 하고 이에 대한 전역 액세스 지점을 제공하는 싱글톤 패턴
2. [팩토리 메소드 패턴](#팩토리-메서드-패턴factory-method) 클래스가 인스턴스화를 서브클래스로 위임할 수 있도록 하는 팩토리 메소드 패턴
3. [추상 팩토리 패턴](#추상-팩토리abstract-factory) 객체의 구체적인 클래스를 지정하지 않고 관련 또는 종속 객체를 생성하기 위한 인터페이스를 제공하는 추상 팩토리 패턴
4. [빌더 패턴](#빌더-패턴builder) 동일한 생성 프로세스가 다른 표현을 생성할 수 있도록 복잡한 객체의 생성을 표현과 분리하는 빌더 패턴
5. [프로토타입 패턴](#프로토타입prototype-패턴) 프로토 타입 인스턴스를 사용하여 생성할 객체의 종류를 지정하고 이 프로토타입을 복제하여 새 객체를 생성하는 프로토타입 패턴
